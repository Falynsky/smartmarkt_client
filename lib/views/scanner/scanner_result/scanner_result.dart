...
class _ScannerResultState extends State<ScannerResult> {
  ...
  Container _scannedProductImage() {
    return Container(
      ...
      child: AspectRatio(
        ...
        child: Container(
          child: Image.network(
            'http://${HttpService.hostUrl}/files/download/${_scannedInfo['documentName']}.${_scannedInfo['documentType']}/db',
  ...
  Widget _scannerInfoPriceRow() {
    return _scannedInfo['afterDiscount'] == null
        ...
        : _productInfoRow("Cena:", "${_scannedInfo['price']}zł", newValue: "${_scannedInfo['afterDiscount']}zł");
  }
  ...
  Widget _addProductToBasketButton(BuildContext context) {
    return Container(
      ...
      child: InkWell(
        ...
        onTap: () {
          final addProductToBasketEvent = AddProductToBasketEvent(productId: _scannedInfo['id']);
